...
<Bars
  title="Boundary-stratum accuracy: the fully-informed global router against the local gate"
  max={1}
  dp={3}
  rows={[
    { n: 'Fast model, triage', v: 0.985 },
    { n: 'Fast model, gate', v: 0.955 },
    { n: 'Strong model, triage', v: 0.985 },
    { n: 'Strong model, gate', v: 0.975 }
  ]}
  note="The gap is 0.03 on the fast model and 0.01 on the strong one, in the wrong direction both
  times. The kill criterion said that if the stateless router matched the gate at boundaries, the
  architecture argument was dead."
/>
...
<Bars
  zero
  dp={4}
  title="Margin of the gate over a tuned sticky baseline, across eight deployments"
  rows={[
    { n: 'Deployment 1', v: -0.0009 },
    { n: 'Deployment 2', v: -0.0015 },
    { n: 'Deployment 3', v: -0.0008 },
    { n: 'Deployment 4', v: -0.0015 },
    { n: 'Deployment 5', v: -0.0037 },
    { n: 'Deployment 6', v: -0.0031 },
    { n: 'Deployment 7', v: 0.0019 },
    { n: 'Deployment 8', v: -0.0060 }
  ]}
  note="One of the eight is positive and it is smaller than the noise. This is what a design that does
  not pay for itself looks like when you have written down in advance what paying would have meant."
/>
...
<Bars
  title="How much of each corpus's state is recoverable from its own visible transcript"
  unit="%"
  max={100}
  rows={[
    { n: 'MultiWOZ, lexical', v: 92, bad: true },
    { n: 'SGD, lexical', v: 92, bad: true },
    { n: 'ABCD, model-graded', v: 43 },
    { n: 'ABCD, lexical', v: 17 }
  ]}
  note="Red is the failure. Where the customer says the state out loud, the stateless router is handed
  the state for free and the experiment you think you are running is not the one running. ABCD is the
  only one of the three whose state is genuinely hidden, because its state is agent actions rather than
...
<Grid
  ...
  hit={0}
  cols={44}
  title="Every ABCD test dialogue. Marked: the ones containing more than one subflow"
  legend="1,004 test dialogues"
  note="There is nothing to find. That is a hard count rather than an estimate."
/>
